#ifndef BF_BLOOM_FILTER_SCALABLE_H
#define BF_BLOOM_FILTER_SCALABLE_H

#include <bf/bloom_filter/basic.h>

namespace bf {

/// The scalable Bloom filter.
class scalable_bloom_filter : public bloom_filter
{
public:
  /// Constructs a scalable Bloom filter.
  /// @param k The number of hash functions in the first level.
  /// @param cells0 The number of cells in the first level.
  /// @param seed0 The seed for the first level.
  scalable_bloom_filter(size_t k, size_t cells, size_t seed = 0);

  using bloom_filter::add;
  using bloom_filter::lookup;

  virtual void add(object const& o) override;
  virtual size_t lookup(object const& o) const override;
  virtual void clear() override;

private:
  /// Appends a new level.
  /// @post `levels_.size() += 1`
  void grow();

  size_t k_;
  size_t cells_;
  size_t seed_;
  std::vector<basic_bloom_filter> levels_;
};

} // namespace bf

#endif
